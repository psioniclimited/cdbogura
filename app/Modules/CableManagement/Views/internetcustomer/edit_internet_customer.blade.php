...
$(document).ready(function () {
    ...
    $('#edit_internet_customer_form').validate({
    ...
    $.get( "{{URL::to('/auto/customerdetails')}}", { customer_id: $('#customer_id').val() } ,function( data ) {
        ...
        subscription_details_id.select2({
            allowClear: true,
            data: [
                { id: data.subscription_detail.id, text:  data.subscription_detail.bandwidth }
            ],
            ajax: {
                dataType: 'json',
                ...
                data: function(params) {
                    return {
                        term: params.term,
                        value_term: subscription_details_type.val(),
                        page: params.page
                    }
                },
                processResults: function (data, params) {
    ...
    //on subscription_details_type change initialize subscription_details_id
...
<section class="content">
    ...
    <div class="box box-info">
        <div class="box-header with-border">
            <h3 class="box-title">Edit Internet Customer</h3>
        </div>
        <!-- /.box-header -->
        <!-- form starts here -->
        ...
        <div class="box-body">
            <div class="col-md-4">
                ...
                <div class="form-group @if ($errors->has('monthly_bill')) has-error @endif">
                    ...
                    <input type="number" class="form-control" name="monthly_bill" id="monthly_bill" placeholder="Enter monthly bill" value="{{$customer->monthly_bill}}" tabindex=3>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="subscription_details_type" id="subscription_details_type" tabindex=5>
                        @for($i = 0; $i < count($subscription_detail_type_array); $i++)
                            <option value="{{$i}}" @if($i == $customer->subscription_detail->shared) selected @endif>{{$subscription_detail_type_array[$i]}}</option>
                        @endfor
                    </select>
                </div>
